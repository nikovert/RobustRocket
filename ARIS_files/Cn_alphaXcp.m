function [Cn_alpha, Xcp, Cda, zeta, Ssm, Ssm_B, Ccm]=Cn_alphaXcp(roro) 
    ...
    beta = sqrt( 1 - min(0.9,M^2)); % M <1
%     beta = sqrt( abs(1 - M^2)); % M <1  %%% BUG FOR BACKWARDS!!! M>1!
...
%     Ccm = (rho/2 * V^2 * roro.A_ref * Cn_alpha) * (Xcp-roro.Xcm);
    Ccm = (rho/2 * V^2 * roro.A_ref * Cn_alpha) * (Xcp_B-roro.Xcm);
%     Ccm = (rho/2 * V^2 * roro.A_ref * Cn_alpha) * (Xcp-roro.Xcm)^2; % SQUAREd as in doc ?!
    Ibody = roro.Ibody;
    zeta = Cda/(2*sqrt(Ccm*Ibody(2,2)));
     
    if (not(isreal(zeta)) && roro.Takeoff)
        disp('whatt?!'); 
        keyboard
    end
